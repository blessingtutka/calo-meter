...
import { ActivityIndicator, Alert, Clipboard, Linking, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';

type ExecuteResultOrUndefined = ExecuteResult | undefined;
type QueryResult = {
    users?: string[];
    value?: string | null; // I add "null" cause in the code there's somewhere where it was given the null value
    map?: [string, string][];
};
...
// The Generique type T was not properly declare to avoid confusion
    ...
    const updateValue = async () => {
        ...
        try {
            ...
            const res = await retryOperation(async () => {
                return await client.execute(account.bech32Address, process.env.EXPO_PUBLIC_USER_MAP_CONTRACT_ADDRESS!, msg, 'auto');
            });
            ...
            Alert.alert('Success', 'Your JSON data has been successfully updated on the blockchain.', [{ text: 'OK' }]);
            ...
            Alert.alert('Error', `Failed to update JSON data: ${errorMessage}. Please check your network connection and try again.`);
        } finally {
            setLoading(false);
            setIsOperationInProgress(false);
                    ...
                    <View style={styles.resultsContainer}>
                            ...
                            <View style={styles.formSection}>
                                ...
                                <View style={styles.buttonRow}>
                                    <TouchableOpacity
                                        ...
                                        style={[
                                            styles.menuButton,
                                            (loading || isOperationInProgress || !!jsonError || isTransactionPending) && styles.disabledButton,
                                        ]}
                        ...
                        {executeResult && (
                            <View style={styles.resultCard}>
                                ...
                                <Text style={styles.resultText}>Transaction Hash: {executeResult.transactionHash}</Text>
                                <Text style={styles.resultText}>Block Height: {executeResult.height}</Text>
                                <TouchableOpacity
                                    onPress={() => {
                                        Linking.openURL(
...
const styles = StyleSheet.create({
    ...
    // add undifined style cause it was not define
    buttonText: {
        color: '#fff',
        fontSize: 14,
        fontWeight: '500',
